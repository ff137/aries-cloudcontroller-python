# coding: utf-8

from __future__ import annotations
from datetime import date, datetime  # noqa: F401

import re  # noqa: F401
from typing import Any, Dict, List, Optional  # noqa: F401

from pydantic import AnyUrl, BaseModel, EmailStr, Field, validator  # noqa: F401
from aries_cloudcontroller.models.v20_cred_bound_offer_request_filter import V20CredBoundOfferRequestFilter
from aries_cloudcontroller.models.v20_cred_preview import V20CredPreview


class V20CredExFree(BaseModel):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.

    V20CredExFree - a model defined in OpenAPI

        auto_remove: The auto_remove of this V20CredExFree [Optional].
        comment: The comment of this V20CredExFree [Optional].
        connection_id: The connection_id of this V20CredExFree.
        credential_preview: The credential_preview of this V20CredExFree [Optional].
        filter: The filter of this V20CredExFree.
        trace: The trace of this V20CredExFree [Optional].
        verification_method: The verification_method of this V20CredExFree [Optional].
    """

    auto_remove: Optional[bool] = Field(alias="auto_remove", default=None)
    comment: Optional[str] = Field(alias="comment", default=None)
    connection_id: str = Field(alias="connection_id")
    credential_preview: Optional[V20CredPreview] = Field(alias="credential_preview", default=None)
    filter: V20CredBoundOfferRequestFilter = Field(alias="filter")
    trace: Optional[bool] = Field(alias="trace", default=None)
    verification_method: Optional[str] = Field(alias="verification_method", default=None)

V20CredExFree.update_forward_refs()
